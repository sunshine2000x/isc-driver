...
static int isc_ioctl_bind(struct isc_handle *isc, void *arg)
{
	...
	if ((acc_mode == O_RDONLY && is_u2k(bind.dir)) ||
	    (acc_mode == O_WRONLY && is_k2u(bind.dir)))
		return -EINVAL;
...

	{
		...
		list_for_each(p, cus->wp)
			m = container_of(p, struct isc_imsg, entry);
	}
	bind.mem = mem->pa;
	bind.size = size;
...
static int isc_ioctl_send(struct isc_handle *isc, void *arg)
{
	...
	if ((isc->fh->f_flags & O_ACCMODE) == O_RDONLY)
		return -EPERM;
...
static int isc_ioctl_recv(struct isc_handle *isc, void *arg)
{
	...
	if ((isc->fh->f_flags & O_ACCMODE) == O_WRONLY)
		return -EPERM;
...
static int isc_flush(struct file *file, fl_owner_t id)
{
	struct isc_handle *isc = file->private_data;
